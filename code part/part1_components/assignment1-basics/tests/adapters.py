...
def _ensure_tiktoken_cache() -> None:
    ...
    cache_dir = project_root / ".tiktoken-cache"
    os.environ.setdefault("TIKTOKEN_CACHE_DIR", str(cache_dir))

    fixtures_dir = project_root / "tests" / "fixtures"
    encoder_fixture = fixtures_dir / "gpt2_vocab.json"
    merges_fixture = fixtures_dir / "gpt2_merges.txt"
    if not encoder_fixture.exists() or not merges_fixture.exists():
        return

    cache_dir.mkdir(exist_ok=True)

    resources = {
        "https://openaipublic.blob.core.windows.net/gpt-2/encodings/main/encoder.json": encoder_fixture.read_bytes(),
        "https://openaipublic.blob.core.windows.net/gpt-2/encodings/main/vocab.bpe": (
            "#version: 0.2\n".encode("utf-8") + merges_fixture.read_bytes()
        ),
    }

    for url, contents in resources.items():
...
def run_save_checkpoint(
    ...
    """
    from cs336_basics.save_load_checkpoint import save_checkpoint
...
def run_load_checkpoint(
    ...
    """
    from cs336_basics.save_load_checkpoint import load_checkpoint
