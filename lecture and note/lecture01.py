...
from references import (
    ...
    bengio2003,
    susketver2014,
    bahdanau2015_attention,
    transformer_2017,
    gpt2,
    t5,
    kaplan_scaling_laws_2020,
    the_pile,
    gpt_j,
    opt_175b,
    bloom,
    ...
    ppo2017,
    muon,
    large_batch_training_2018,
    wsd_2024,
    cosine_learning_rate_2017,
    olmo_7b,
    moe_2017,
    megatron_lm_2019,
    shazeer_2020,
...
def why_this_course_exists():
    ...
    response = query_gpt4o(
        prompt="Why teach a course on building language models from scratch? Answer in one sentence."
    )  # @inspect response
    ...
    image(
        "https://upload.wikimedia.org/wikipedia/commons/thumb/c/cc/Industrialisation.jpg/440px-Industrialisation.jpg",
        width=400,
    )
    ...
    ), article_link("https://openai.com/index/announcing-the-stargate-project/")
    ...
    ), x_link("https://x.com/stephenroller/status/1579993017234382849")
    image("images/roller-flops.png", width=400)
    text("Example 2: emergence of behavior with scale "), link(
...
def current_landscape():
    ...
    text("- Sequence-to-sequence modeling (for machine translation) "), link(
        susketver2014
    )
    text("- Adam optimizer "), link(adam2014)
    text("- Attention mechanism (for machine translation) "), link(
        bahdanau2015_attention
    )
    text("- Transformer architecture (for machine translation) "), link(
        transformer_2017
    )
    text("- Mixture of experts "), link(moe_2017)
    text("- Model parallelism "), link(gpipe_2018), link(zero_2019), link(
        megatron_lm_2019
    )
    ...
    text("- Scaling laws: provide hope / predictability for scaling "), link(
        kaplan_scaling_laws_2020
    )
    ...
    text("- DeepMind's Chinchilla (70B): compute-optimal scaling laws "), link(
        chinchilla
    )
    text("## Open models")
    text("- EleutherAI's open datasets (The Pile) and models (GPT-J) "), link(
        the_pile
    ), link(gpt_j)
    text("- Meta's OPT (175B): GPT-3 replication, lots of hardware issues "), link(
        opt_175b
    )
    ...
    text("- DeepSeek's models "), link(deepseek_67b), link(deepseek_v2), link(
        deepseek_v3
    )
    ...
    text("- Anthropic's Claude Sonnet 3.7 "), link(
    ...
    text("- Alibaba's Qwen 2.5 Max "), link(
        "https://qwenlm.github.io/blog/qwen2.5-max/"
    )
    text("- Tencent's Hunyuan-T1 "), link(
...
def course_logistics():
    text("All information online: "), link(
...
def basics():
    ...
    text("Tokenizer-free approaches: "), link(byt5), link(megabyte), link(blt), link(
        tfree
    )
    text(
    ...
    text("- Normalization: LayerNorm, RMSNorm "), link(layernorm_2016), link(
        rms_norm_2019
    )
    text("- Placement of normalization: pre-norm versus post-norm "), link(
        pre_post_norm_2020
    )
    text("- MLP: dense, mixture of experts "), link(moe_2017)
    text("- Attention: full, sliding window, linear "), link(mistral_7b), link(
    ...
    text("## Training")
    text("- Optimizer (e.g., AdamW, Muon, SOAP) "), link(adam2014), link(
        adamw2017
    ), link(muon), link(soap)
    text("- Learning rate schedule (e.g., cosine, WSD) "), link(
        cosine_learning_rate_2017
    ), link(wsd_2024)
    ...
    text("## Assignment 1")
    link(
        title="[GitHub]", url="https://github.com/stanford-cs336/assignment1-basics"
    ), link(
        title="[PDF]",
...
def systems():
    ...
    image(
        "https://miro.medium.com/v2/resize:fit:2000/format:webp/1*6xoBKi5kL2dZpivFe1-zgw.jpeg",
        width=800,
    )
...
def scaling_laws():
    ...
    text("Compute-optimal scaling laws: "), link(kaplan_scaling_laws_2020), link(
        chinchilla
    )
...
def data():
    ...
    image(
        "https://ar5iv.labs.arxiv.org/html/2101.00027/assets/pile_chart2.png", width=600
    )
    text("## Evaluation")
    ...
    text("- Appeal to fair use to train on copyright data? "), link(
...
def learning_from_feedback():
    ...
    preference_data: list[PreferenceExample] = [
        PreferenceExample(
            history=[
                ...
                Turn(
                    role="user",
                    content="What is the best way to train a language model?",
                ),
            ],
    ...
    text("## Algorithms")
    text("- Proximal Policy Optimization (PPO) from reinforcement learning "), link(
        ppo2017
    ), link(instruct_gpt)
...
def tokenization():
    ...
    ), youtube_link("https://www.youtube.com/watch?v=zduSFxRajkE")
...
def tokenization_examples():
    text("To get a feel for how tokenizers work, play with this "), link(
        title="interactive site", url="https://tiktokenizer.vercel.app/?encoder=gpt2"
    )
    text("## Observations")
    text(
...
def character_tokenizer():
    ...
    reconstructed_string = tokenizer.decode(indices)  # @inspect reconstructed_string
    assert string == reconstructed_string
    text("There are approximately 150K Unicode characters. "), link(
        title="[Wikipedia]",
...
def byte_tokenizer():
    ...
    text("The most common Unicode encoding is "), link(
        title="UTF-8", url="https://en.wikipedia.org/wiki/UTF-8"
    )
...
def bpe_tokenizer():
    ...
    ), article_link("http://www.pennelynn.com/Documents/CUJ/HTML/94HTML/19940045.HTM")
