...
def motivating_questions():
    ...
    mfu = 0.5
    flops_per_day = (
        h100_flop_per_sec * mfu * 1024 * 60 * 60 * 24
    )  # @inspect flops_per_day
    ...
    bytes_per_parameter = (
        4 + 4 + (4 + 4)
    )  # parameters, gradients, optimizer state  @inspect bytes_per_parameter
...
def tensors_memory():
    ...
    assert (
        get_memory_usage(torch.empty(12288 * 4, 12288)) == 2304 * 1024 * 1024
    )  # 2.3 GB
    text("...which is a lot!")
    text("## float16")
    link(
    ...
    text("- Solution (later): use mixed precision training, see "), link(
        mixed_precision_training
    )
...
def tensor_slicing():
    ...
    text("Get row 0:")
    y = x[0]  # @inspect y
    assert torch.equal(y, torch.tensor([1.0, 2, 3]))
...
def jaxtyping_basics():
    ...
    x: Float[torch.Tensor, "batch seq heads hidden"] = torch.ones(
        2, 2, 1, 3
    )  # @inspect x
...
def einops_einsum():
    ...
    text("New (einops) way:")
    z = einsum(
        x, y, "batch seq1 hidden, batch seq2 hidden -> batch seq1 seq2"
    )  # @inspect z
...
def tensor_operations_flops():
    ...
    text("## Intuitions")
    text("Training GPT-3 (2020) took 3.14e23 FLOPs. "), article_link(
        "https://lambdalabs.com/blog/demystifying-gpt-3"
    )
    text("Training GPT-4 (2023) is speculated to take 2e25 FLOPs "), article_link(
    ...
    text("A100 has a peak performance of 312 teraFLOP/s "), link(
        title="[spec]",
    ...
    promised_flop_per_sec = get_promised_flop_per_sec(
        device, x.dtype
    )  # @inspect promised_flop_per_sec
    ...
    bf16_actual_flop_per_sec = (
        actual_num_flops / bf16_actual_time
    )  # @inspect bf16_actual_flop_per_sec
    bf16_promised_flop_per_sec = get_promised_flop_per_sec(
        device, x.dtype
    )  # @inspect bf16_promised_flop_per_sec
    bf16_mfu = (
        bf16_actual_flop_per_sec / bf16_promised_flop_per_sec
    )  # @inspect bf16_mfu
...
def gradients_basics():
    ...
    w = torch.tensor([1.0, 1, 1], requires_grad=True)  # Want gradient
    pred_y = x @ w
...
def gradients_flops():
    ...
    text("A nice graphical visualization: "), article_link(
...
def module_parameters():
    ...
    text("Up to a constant, this is Xavier initialization. "), link(
        title="[paper]", url="https://proceedings.mlr.press/v9/glorot10a/glorot10a.pdf"
    ), link(
        title="[stackexchange]",
...
def optimizer():
    ...
    text("Blog post describing memory usage for Transformer training "), article_link(
        "https://erees.dev/transformer-memory/"
    )
    text("Blog post descibing FLOPs for a Transformer: "), article_link(
...
def mixed_precision_training():
    ...
    text("Use FP8 pervasively throughout training "), link(
